...
for n in $(( SLURM_JOB_NUM_NODES - 1 )); do
  for shard in ${NSHARD:-4}; do
    for py in ${NPY:-4}; do
      while true; do
        start_time="$(date)"
        echo "Starting attempt $i on $start_time"
        export LIMIT=$(( 4 * n * shard * py ))

        export MAX_TASKS_IN_FLIGHT=$(( 1280 / shard / py ))

        export OUT_DIR=$SCRATCH/cori-cctbx.try-subprocess-2019-06-lifeline/output_legion_"$SLURM_JOB_ID"_n_${n}_shard_${shard}_py_${py}_io_1_attempt$i
        mkdir -p $OUT_DIR
        mkdir -p $OUT_DIR/backtrace
        echo "$start_time" > "$OUT_DIR/timestamp"
